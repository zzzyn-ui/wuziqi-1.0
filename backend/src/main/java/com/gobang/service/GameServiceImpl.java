...
public class GameServiceImpl implements GameService {
    ...
    public void makeMove(Long userId, String roomId, int x, int y) {
        ...
        log.info("🎮 makeMove: roomId={}, userId={}, x={}, y={}, currentTurn={}, blackPlayerId={}, whitePlayerId={}",
                roomId, userId, x, y, room.getCurrentTurn(), room.getBlackPlayerId(), room.getWhitePlayerId());

        // 检查是否轮到该玩家
        if (room.getCurrentTurn() != userId) {
            log.warn("❌ 不是你的回合: userId={}, currentTurn={}, pieceColor={}",
                    userId, room.getCurrentTurn(), room.getPieceColor(userId));
        ...
        if (hasWon) {
            ...
            // 切换回合
            Long oldTurn = room.getCurrentTurn();
            room.switchTurn();
            Long newTurn = room.getCurrentTurn();
            log.info("🔄 切换回合: roomId={} {} -> {}, blackPlayerId={}, whitePlayerId={}",
                    roomId, oldTurn, newTurn, room.getBlackPlayerId(), room.getWhitePlayerId());
            // 广播游戏状态
    ...
    public String createRoom(Long creatorId, Long blackPlayerId, String mode) {
        ...
        // 将黑棋玩家也添加到 RoomManager 的玩家列表中
        roomManager.joinRoom(roomId, blackPlayerId);
    ...
    private void handleGameOver(GameRoom room, Long winnerId, int winColor, int endReason) {
        ...
        int moveCount = room.getMoveCount();
        List<int[]> moveHistory = room.getMoveHistory();
        log.info("保存对局记录: roomId={}, moveCount={}, moveHistory大小={}",
                room.getRoomId(), moveCount, moveHistory == null ? "null" : moveHistory.size());
        ...
        record.setMoves(serializeMoves(moveHistory));
    ...
    private void broadcastGameState(GameRoom room) {
        ...
        // 计算当前回合的颜色
        int currentTurnColor = room.getPieceColor(room.getCurrentTurn());
        state.put("currentTurn", currentTurnColor);  // 发送颜色 (1或2) 而不是 userId

        log.info("🎯 [broadcastGameState] currentTurn详情: userId={}, color={}, blackPlayerId={}, whitePlayerId={}",
                room.getCurrentTurn(), currentTurnColor, room.getBlackPlayerId(), room.getWhitePlayerId());
     ...
     * 序列化走棋记录为JSON格式
     */
    private String serializeMoves(List<int[]> moveHistory) {
        log.info("serializeMoves 被调用, moveHistory大小: {}", moveHistory == null ? "null" : moveHistory.size());
        if (moveHistory == null || moveHistory.isEmpty()) {
            log.warn("moveHistory 为空，返回空数组");
            return "[]";
        }
        StringBuilder sb = new StringBuilder();
        sb.append("[");
        for (int i = 0; i < moveHistory.size(); i++) {
            ...
            sb.append("{\"x\":").append(move[0]).append(",\"y\":").append(move[1]).append("}");
        ...
        for (Map.Entry<String, GameRoom> entry : rooms.entrySet()) {
            ...
            if (userId.equals(room.getBlackPlayerId()) || userId.equals(room.getWhitePlayerId())) {
                ...
                if (isCreator || !isGameFinished) {
                    log.info("🗑️ 删除房间: roomId={}, userId={}, isCreator={}, isGameFinished={}",
                            roomId, userId, isCreator, isGameFinished);
                    rooms.remove(roomId);
    ...
    public static class GameRoom {
        ...
        public GameRoom(String roomId, Long creatorId, Long blackPlayerId, String mode) {
            ...
            this.whitePlayerId = creatorId; // creatorId 是白棋玩家
        ...
        public void switchTurn() {
            ...
            Long oldTurn = currentTurn;
            currentTurn = currentTurn.equals(blackPlayerId) ? whitePlayerId : blackPlayerId;
            log.info("🔄 switchTurn: {} -> {} (blackPlayerId={}, whitePlayerId={})",
                    oldTurn, currentTurn, blackPlayerId, whitePlayerId);
        }
