...
public class ObserverServiceImpl implements ObserverService {
    ...
    public List<Map<String, Object>> getObservableRooms() {
        ...
        // 获取所有进行中的房间（有2个玩家的房间）
        for (RoomManager.RoomInfo roomInfo : roomManager.getObservableRooms()) {
            String roomId = roomInfo.getRoomId();

            // 检查游戏是否正在进行（未结束）
            Object gameRoom = gameService.getGameRoom(roomId);
            if (gameRoom == null) {
                logger.debug("房间 {} 的 GameRoom 不存在，跳过", roomId);
                continue;
            }

            // 使用反射检查游戏状态
            String gameStateStr = null;
            int moveCount = 0;
            ...
            try {
                ...
                java.lang.reflect.Method isGameOverMethod = gameRoom.getClass().getMethod("isGameOver");
                isGameOver = (Boolean) isGameOverMethod.invoke(gameRoom);

                java.lang.reflect.Method getMoveCountMethod = gameRoom.getClass().getMethod("getMoveCount");
                moveCount = (Integer) getMoveCountMethod.invoke(gameRoom);
            } catch (Exception e) {
                logger.error("检查游戏状态失败: roomId={}", roomId, e);
                continue;
            }

            // 只显示游戏正在进行中（PLAYING）且有落子的房间
            if (!"PLAYING".equals(gameStateStr) || isGameOver) {
                logger.debug("房间 {} 游戏状态={}, isGameOver={}, 不显示在可观战列表中", roomId, gameStateStr, isGameOver);
                continue;
            }

            // 额外检查：至少有一步落子才算真正开始
            if (moveCount == 0) {
                logger.debug("房间 {} 还没有落子，不显示在可观战列表中", roomId);
                continue;
            }
            ...
            roomData.put("observerCount", getObserverCount(roomId));
            roomData.put("moveCount", moveCount);
        ...
        logger.info("获取可观战房间列表: {} 个房间", observableRooms.size());
