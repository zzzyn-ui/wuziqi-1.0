...
public class SpringMatchMaker {
    ...
    /**
     * 从所有队列中移除玩家（用于 WebSocket 断开时清理）
     */
    public synchronized void cancelMatchAll(Long userId) {
        logger.info("🧹 从所有匹配队列中移除玩家: {}", userId);
        ...
        for (Map.Entry<String, ConcurrentLinkedQueue<MatchRequest>> entry : waitingPlayers.entrySet()) {
            ...
            if (sizeBefore != sizeAfter) {
                ...
                logger.info("✓ 玩家 {} 已从 {} 队列移除 ({} 个请求)", userId, mode, sizeBefore - sizeAfter);
            }
        }

        if (removedCount > 0) {
            logger.info("✓ 总共移除了玩家 {} 的 {} 个匹配请求", userId, removedCount);
            printQueueStatus();
        }
    }
    ...
    private void createAndNotifyGame(MatchRequest player1, MatchRequest player2, String mode) {
        try {
            ...
            boolean player1IsBlack = random.nextBoolean();

            MatchRequest blackPlayer = player1IsBlack ? player1 : player2;
            MatchRequest whitePlayer = player1IsBlack ? player2 : player1;

            logger.info("🎲 随机分配颜色: {} ({}) 执黑, {} ({}) 执白",
                    blackPlayer.userId, blackPlayer.username,
                    whitePlayer.userId, whitePlayer.username);

            // 使用白棋玩家作为创建者（但黑棋是随机指定的）
            String roomId = gameService.createRoom(whitePlayer.userId, blackPlayer.userId, mode);

            // 白棋玩家已经在创建时加入了，黑棋玩家也已经加入
            // 通知两个玩家匹配成功
            notifyMatchSuccess(blackPlayer.userId, roomId, 1, whitePlayer.userId, whitePlayer.username);
            notifyMatchSuccess(whitePlayer.userId, roomId, 2, blackPlayer.userId, blackPlayer.username);

            logger.info("🏠 房间创建成功: roomId={}, 黑棋={}, 白棋={}, 模式={}, 积分影响={}",
                    roomId, blackPlayer.userId, whitePlayer.userId,
