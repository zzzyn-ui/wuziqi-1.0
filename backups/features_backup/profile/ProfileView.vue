...
import { ref, reactive, onMounted, onUnmounted, watchEffect, onActivated } from 'vue'
...
watchEffect(() => {
  if (userStore.userInfo && (userStore.userInfo.nickname || userStore.userInfo.username)) {
    const newNickname = userStore.userInfo.nickname || userStore.userInfo.username || ''
    // 只在表单为空时自动填充，避免覆盖用户正在编辑的内容
    if (!profileForm.nickname || profileForm.nickname === '') {
      profileForm.nickname = newNickname
    }
  }
})
</script>
