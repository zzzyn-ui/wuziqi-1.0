  ...
  const roomSubscription = wsClient.subscribe(`/topic/room/${roomId.value}`, (message: any) => {
    ...
    switch (message.type) {
      ...
      case 'UNDO_RESPONSE':
        ...
        if (message.accepted) {
          ElMessage.success('对手同意悔棋')
        } else {
          ElMessage.info('对手拒绝悔棋')
        }
        // 清理悔棋弹窗
        if (undoMessageBoxInstance) {
          try {
      ...
      case 'DRAW_RESPONSE':
        ...
        if (message.requesterId === userStore.userInfo?.id) {
          if (message.accepted) {
            ElMessage.success('对手同意和棋')
          } else {
            ElMessage.info('对手拒绝和棋')
          }
        }
        // 清理和棋弹窗
        if (drawMessageBoxInstance) {
