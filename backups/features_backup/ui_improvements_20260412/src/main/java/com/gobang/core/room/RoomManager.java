...
public class RoomManager {
    ...
    /**
     * 获取所有可观战房间（已开始，有2个玩家，且游戏未结束）
     * 注意：这里只返回房间信息，实际的游戏状态需要从 GameService 获取
     */
    public List<RoomInfo> getObservableRooms() {
        List<RoomInfo> observableRooms = new ArrayList<>();
        for (RoomInfo room : rooms.values()) {
            // 只返回公开的、有2个玩家的房间
            if (!room.isPrivate() && room.getPlayerCount() >= 2) {
    ...
    /**
     * 清理所有空房间（立即删除所有空房间，不管是否过期）
     */
    public void cleanAllEmptyRooms() {
        int beforeSize = rooms.size();
        rooms.entrySet().removeIf(entry -> {
            RoomInfo room = entry.getValue();
            return room.getPlayerCount() == 0;
        });
        int afterSize = rooms.size();
        logger.info("清理所有空房间: 删除了 {} 个空房间", beforeSize - afterSize);
    }

    /**
     * 清理所有房间（删除所有房间，包括有玩家的）
     */
    public void clearAllRooms() {
        int beforeSize = rooms.size();
        rooms.clear();
        logger.info("清理所有房间: 删除了 {} 个房间", beforeSize);
    }
