...
const handleChat = (friend: Friend) => {
  ...
  // 订阅私聊消息
  privateMessageSubscription = wsClient.subscribePrivateMessage((data) => {
  ...
  // 订阅聊天历史响应
  wsClient.subscribeChatHistory((data) => {
    console.log('[FriendsView] 收到聊天历史:', data)
    if (data.type === 'CHAT_HISTORY' && data.friendId === friend.userId) {
      // 反转消息顺序（从旧到新）
      chatMessages.value = (data.messages || []).reverse()
      nextTick(() => {
        scrollToBottom()
      })
    }
