# 更新日志

本文档记录五子棋在线对战系统的所有重要变更。

格式基于 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.0.0/)，
版本号遵循 [语义化版本](https://semver.org/lang/zh-CN/)。

---

## [未发布]

### 计划中
- 支持自定义棋盘大小
- 添加更多残局关卡
- 移动端适配优化
- 主题系统（暗色模式）

---

## [1.2.2] - 2026-04-14

### 新增
- ✨ **AI强度提升**: 大幅提升人机对战AI水平
  - 简单模式：添加基本胜利检测和防守逻辑
  - 中等模式：实现位置评分系统，识别活四、冲四、活三等棋形
  - 困难模式：增强攻防权重，更精确的棋形识别
  - 专家模式：新增极难挑战模式
- ✨ **核心AI算法**: 新增智能评估函数
  - `evaluatePosition`: 综合评估位置分数
  - `evaluatePoint`: 评估某位置的棋形分数
  - `getLineInfo`: 获取某方向上的棋形信息
  - `getLineScore`: 根据棋形给分

### 改进
- ♻️ **AI决策**: AI现在能主动构建威胁并防守玩家进攻
- ♻️ **位置选择**: AI优先考虑战略位置，避免孤立落子
- ♻️ **攻防平衡**: 不同难度使用不同的攻防权重

### 修复
- 🐛 **AI先手**: 修复AI先手第一步落子过于随机的问题

---

## [1.2.1] - 2026-04-12

### 新增
- ✨ **代码结构文档**: 新增详细的代码结构说明文档 (CODE_STRUCTURE.md)
- ✨ **清理报告**: 新增代码清理报告文档 (CLEANUP_REPORT.md)

### 修复
- 🐛 **聊天历史加载**: 修复聊天历史不能每次加载的问题
  - 将聊天历史从 user-queue 改为 topic-based 方式
  - 后端使用 `/topic/chat/history/{userId}` 代替 `convertAndSendToUser`
  - 前端订阅改为 `/topic/chat/history/${userId}`
- 🐛 **重复消息**: 修复发送者看到两条聊天消息的问题

### 改进
- ♻️ **UI 优化**: 移除页脚的在线玩家和今日对局显示
- ♻️ **UI 优化**: 移除排行榜的在线状态显示
- ♻️ **代码清理**: 删除多余代码、日志文件和临时文件
- 📝 **文档更新**: 更新所有项目文档以反映当前架构

### 技术变更
- 后端框架从 Netty 迁移到 Spring Boot 3 + Spring WebSocket
- 前端从原生 JavaScript 迁移到 Vue 3 + TypeScript
- WebSocket 协议从自定义协议改为 STOMP

---

## [1.2.0] - 2024-03-01

### 新增
- **残局系统**: 新增残局闯关模式，包含简单/中等/困难三个难度
- **残局记录**: 记录用户残局通关记录和星级评定
- **残局排行榜**: 新增残局通关排行榜
- **好友私聊**: 支持好友之间一对一聊天
- **未读消息**: 新增未读消息数量查询接口

### 改进
- 优化匹配算法，减少匹配等待时间
- 改进棋盘渲染性能
- 增强错误提示信息

### 修复
- 修复观战模式下棋盘状态不同步问题
- 修复断线重连后积分计算错误
- 修复好友列表在线状态显示问题

---

## [1.1.0] - 2024-02-01

### 新增
- **观战功能**: 支持观战正在进行的对局
- **对局回放**: 支持历史对局完整回放
- **游戏邀请**: 支持向好友发送游戏邀请
- **房间系统**: 支持创建和加入私人房间
- **个人收藏**: 支持收藏精彩对局

### 改进
- 优化 WebSocket 连接稳定性
- 改进 ELO 积分算法
- 增加匹配超时机制

### 修复
- 修复匹配队列偶尔卡死的问题
- 修复聊天消息丢失问题
- 修复排行榜数据刷新延迟

---

## [1.0.1] - 2024-01-15

### 修复
- 修复 JWT token 过期处理问题
- 修复数据库连接池泄露问题
- 修复密码重置功能异常
- 修复移动端页面布局问题

### 改进
- 增加服务器监控指标
- 优化日志输出格式
- 改进错误处理机制

---

## [1.0.0] - 2024-01-01

### 新增
- **用户系统**: 用户注册、登录、认证
- **匹配系统**: 基于 ELO 积分的智能匹配
- **对战功能**: 双人实时在线对战
- **人机对战**: 支持 PvE 模式
- **聊天系统**: 公共聊天室
- **好友系统**: 添加好友、好友列表
- **排行榜**: 积分排行榜
- **对局记录**: 历史对局记录查询
- **积分系统**: ELO 积分计算
- **等级系统**: 基于积分的等级划分

### 技术栈
- 后端: Java 17 + Spring Boot 3.x
- 前端: Vue 3 + TypeScript
- 数据库: MySQL 8.0
- 缓存: Redis 7.0
- 认证: JWT
- 协议: WebSocket (STOMP) + HTTP REST API

---

## 版本说明

### 版本号规则

- **主版本号**: 不兼容的 API 变更
- **次版本号**: 向下兼容的功能新增
- **修订号**: 向下兼容的问题修复

### 更新类型

- **新增**: 新功能
- **改进**: 对现有功能的改进
- **修复**: 问题修复
- **安全**: 安全相关的修复
- **弃用**: 即将移除的功能

---

## 迁移指南

### 从 1.1.x 升级到 1.2.1

#### 后端变更
```java
// WebSocket 消息处理器变更
// 旧版本: 自定义消息协议
// 新版本: STOMP 协议

// 聊天历史订阅变更
// 旧版本: convertAndSendToUser(userId, "/queue/chat/history", response)
// 新版本: convertAndSend("/topic/chat/history/" + userId, response)
```

#### 前端变更
```typescript
// WebSocket 连接变更
// 旧版本: 原生 WebSocket
// 新版本: @stomp/stompjs + SockJS

// 聊天历史订阅变更
// 旧版本: subscribe('/user/queue/chat/history', callback)
// 新版本: subscribe('/topic/chat/history/${userId}', callback)
```

### 从 1.0.x 升级到 1.2.1

由于从 Netty 迁移到 Spring Boot，需要完整重新部署：

1. 备份数据库
2. 停止旧版本服务
3. 部署新版本代码
4. 运行数据库迁移脚本
5. 启动新版本服务

详细步骤请参考 [QUICK_DEPLOY.md](QUICK_DEPLOY.md)

---

## 贡献指南

如果你发现了 bug 或有新功能建议，请：

1. 查阅 [贡献指南](CONTRIBUTING.md)
2. 创建 Issue 描述问题
3. 提交 Pull Request

---

## 致谢

感谢所有贡献者和用户的支持！

---

## 许可证

[MIT License](../LICENSE)
