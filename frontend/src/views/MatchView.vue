  ...
  gameSubscription.value = wsClient.subscribe(`/topic/room/${roomId.value}`, (data: any) => {
    ...
    switch (data.type) {
      case 'GAME_STATE':
        ...
        console.log('[Match] GAME_STATE 详细信息:', {
          hasBlackPlayer: !!data.blackPlayer,
          hasWhitePlayer: !!data.whitePlayer,
          currentTurn: data.currentTurn,
          gameStatus: data.gameStatus,
          board: data.board ? '有' : '无'
        })
        ...
        // 更新回合
        if (data.currentTurn) {
          console.log('[Match] 更新回合:', currentTurn.value, '->', data.currentTurn)
          currentTurn.value = data.currentTurn
        } else {
          console.log('[Match] ⚠️ GAME_STATE 中没有 currentTurn 字段!')
        }
...
const handleMove = (x: number, y: number) => {
  console.log('[Match] ==================== handleMove ====================')
  console.log('[Match] 点击位置:', { x, y })
  console.log('[Match] 当前回合 currentTurn:', currentTurn.value, '(1=黑, 2=白)')
  console.log('[Match] 我的颜色 myColor:', myColor.value, '(1=黑, 2=白)')
  console.log('[Match] 是否轮到我:', currentTurn.value === myColor.value)

  if (currentTurn.value !== myColor.value) {
    console.log('[Match] ❌ 不是我的回合，无法落子')
