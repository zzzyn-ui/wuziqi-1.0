      ...
      <template #footer>
        <div class="game-result-footer">
          ...
          <div class="play-again-button-wrapper">
            <div v-if="playAgainStatus.opponentRequested" class="opponent-ready-tip">
              对手已准备
            </div>
...
const startMatch = async (mode: 'casual' | 'ranked', avoidOpponentId?: number) => {
  ...
  try {
    ...
    // 订阅成功后再发送匹配请求
    // 所有对局都使用ranked模式计算积分，但记住用户选择的模式用于结算显示
    console.log('[Match] 发送匹配请求:', mode, avoidOpponentId)
    const payload: any = { mode: 'ranked' } // 始终使用ranked模式
    if (avoidOpponentId) {
  ...
  gameSubscription.value = wsClient.subscribe(`/topic/room/${roomId.value}`, (data: any) => {
    ...
    switch (data.type) {
      ...
      case 'CHANGE_TABLE':
        ...
        ElMessage.info('对手选择了换桌')
        // 延迟一下再换桌，让用户看到消息
        setTimeout(() => {
          handleChangeTable(false) // false 表示不再次发送换桌消息
        }, 1000)
        break

      case 'OPPONENT_LEFT':
        console.log('[Match] 对手离开了')
        ElMessage.info('对手离开了游戏')
        // 对手离开后，自动换桌
        setTimeout(() => {
          handleChangeTable(false) // false 表示不发送换桌消息（对手已经离开了）
        }, 1500)
        break
